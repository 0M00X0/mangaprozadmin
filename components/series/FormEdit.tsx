...
export default function SeriesFormEdit() {
    ...
    useEffect(() => {
        if (series) {
            setFieldValue("seriesname", series.seriesname);
            setFieldValue("seriesslug", series.seriesslug);
            setseriesslug(siteUrl + "/series/" + series.seriesslug);
            setFieldValue("seriesdescription", series.seriesdescription);
            setFieldValue("seriesalternativenames", series.seriesalternativenames);
            setFieldValue("seriesauthor", series.seriesauthor);
            setFieldValue("seriesartist", series.seriesartist);
            setFieldValue("seriesyear", series.seriesyear);
            setFieldValue("seriesserialization", series.seriesserialization);
            setFieldValue("seriesscore", series.seriesscore);
            setFieldValue("seriestype", series.seriestype);
            setFieldValue("seriesstatus", series.seriesstatus);
            setFieldValue("seriesgenres", series.seriesgenres);
            setSelectedGenres(series.seriesgenres);
            setFieldValue("seriesthumbnail", series.seriesthumbnail);
            setSelectedImageThumbnail(series.seriesthumbnail);
            setFieldValue("seriescover", series.seriescover);
            setSelectedImageCover(series.seriescover);

        }
    }, [series, setFieldValue, siteUrl]);
    ...
    const addwhitaxios = async () => {
        axios.put(`/api/series/edit/${id}`, { data })
            .then((res) => {
                console.log(res);
    ...
    function uploadimage(ev: any) {
        ev.preventDefault();
        const selectedFileCoverCheck = selectedFileCover;
        const selectedFileThumbnailCheck = selectedFileThumbnail;
        try {
            if (selectedFileCoverCheck) {
                handleImageUploadCover();
            } else if (selectedFileThumbnailCheck) {
                                                    ...
                                                    <div className="w-full">
                                                        <Multiselect
                                                            ...
                                                            onSelect={(selectedList) => {
                                                                const selectedGenres = selectedList.map((genre: any) => JSON.stringify({
                                                                    id: genre.id,
                                                                    name: genre.name
                                                                }));
                                ...
                                <div className="text-gray-800 mb-2 flex flex-row justify-between">
                                    ...
                                    <div>
                                        <button
                                            ...
                                            disabled={
                                                !values.seriescover || !values.seriesthumbnail || !values.seriesslug
                                                    ? true
                                                    : false
                                            }
                                            type="submit"
